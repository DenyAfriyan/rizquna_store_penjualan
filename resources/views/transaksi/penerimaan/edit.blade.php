        ...
        <div class="card">
            ...
            <div class="card-body">
                @if($errors->any())
                  ...
                  @endif
                <form action="{{ route('penerimaan.update' , ['penerimaan' => $penerimaan->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        ...
                        <select class="select2 form-control" name="jenis_limbah_id" id="jenis_limbah_id">
                            {{-- @foreach ($jenis_limbah as $key => $val) --}}
                                <option value="{{ $penerimaan->jenis_limbah_id }}" >{{ $penerimaan->jenis_limbah->name }}</option>
                            {{-- @endforeach --}}
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="select2 form-control" name="sumber_limbah_id" id="sumber_limbah_id" >
                            @foreach ($sumber_limbah as $key => $val)
                                <option value="{{ $val }}" {{ $val == $penerimaan->sumber_limbah_id ? 'selected' : '' }}>{{ $key }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control" id="jumlah_limbah_masuk" name="jumlah_limbah_masuk" value="{{ $penerimaan->jumlah_limbah_masuk }}">
                            ...
                            <div class="col-lg-6">
                                ...
                                <input type="datetime-local" class="form-control" id="tanggal_masuk" name="tanggal_masuk" value="{{ $penerimaan->tanggal_masuk }}">
                            </div>
                            <div class="col-lg-6">
                                <label for="maksimal_penyimpanan">Tanggal Maksimal Penyimpanan:</label>
                                <input type="datetime-local" class="form-control" id="maksimal_penyimpanan" name="maksimal_penyimpanan" value="{{ $penerimaan->maksimal_penyimpanan }}">
                            </div>
                        </div>
